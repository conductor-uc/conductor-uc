...
| Connects to | Redis (rate-limit counters, required); identity, org, pbx-config, callflow, voicemail, cdr, trunk and recording services over HTTP; org-service's internal certificate, ACME-challenge and org-lineage routes; identity-service's permission lookup, call-control's live calls and pbx-config-service's "whose extension" lookup (for the realtime hub); call-control for the recording buttons on live calls (S5-15); **NATS** (the realtime hub reads `call.*` events and publishes audit records; not required to start: the gateway serves the API without it and keeps retrying). |
...
| **call-control** | Connects to every FreeSWITCH event socket, tracks live calls and node health in Redis, hands out the leases that pin a queue, parking lot or conference to one node, and carries out the recording buttons on live calls (S5-15) | `call_control` (outbox only); Redis: calls, nodes, leases | **FreeSWITCH ESL on every node**; Redis; NATS; identity, recording and pbx-config services (the recording buttons) | ESL connections; heartbeats; lease renewals; relay |
