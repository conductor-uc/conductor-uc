  ...
  () => {
    ...
    describe('retention settings', () => {
      ...
      it('reports the default, stores a choice, sets the bucket rule, and audits it', async () => {
        const headers = admin();
        expect((await send('GET', url, headers)).json()).toEqual({ retentionDays: 90 });

        const response = await send('PUT', url, headers, { retentionDays: 14 });
        expect(response.statusCode).toBe(200);
        expect(response.json()).toEqual({ retentionDays: 14 });
        expect((await send('GET', url, headers)).json()).toEqual({ retentionDays: 14 });
