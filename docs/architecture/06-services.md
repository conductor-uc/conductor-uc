...
**Emits:** `call.channel.created|answered|bridged|held|unheld|recording_started|recording_stopped|hungup` (with the tenant in `orgContext` whenever it is known: a call from a trunk has none at `created`, and gets it from `cuc_tenant_id` on its later events), `call.lost`, `call.queue.*` (from `mod_callcenter` events), `call.conference.*`, `call.park.*`. Events are rate-shaped per tenant.
