...
| Connects to | telephony-config over HTTP: every call's dialplan, directory and module configuration, call-flow definitions, prompts, voicemail and conference PINs. cdr-service over HTTP (one JSON record per call). Redis (a per-tenant concurrent-call counter). OpenSIPs (outbound legs). Object storage (voicemail messages are uploaded straight to a presigned URL). |
