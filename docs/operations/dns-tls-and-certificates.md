...
**CORS (G-80).** The console uploads recordings and brand images straight to the bucket, which is a cross-origin request from the console's hostname. The platform sets the bucket's CORS rule itself, on every bucket it creates and again on existing buckets the first time each service touches them after it starts: `PUT`, `GET` and `HEAD` from any origin, the `Content-Type` header, no credentials. Any origin is safe because the rule grants nothing on its own; every request still needs its own signed URL for one object, valid for at most 15 minutes. There is nothing to configure on a hosted store as long as the key may set the CORS configuration (`s3:PutBucketCORS`, see configuration-reference §3.6). If it may not, each service logs one warning when it starts using storage (`The storage provider refused to set the browser CORS rule (AccessDenied)`), stops trying, and browser uploads fail until you grant the permission and restart, or add the rule by hand. Self-hosted MinIO does not take a per-bucket rule (it answers `NotImplemented`, which is logged the same way, once per service) and allows any origin by default, so keep its `MINIO_API_CORS_ALLOW_ORIGIN` at `*` or set it to your console hostnames.
