...
| S5-14 | Done | Agent-scoped recording rules for queue calls (G-111): policy scope `agent` (extension > agent > queue > DID > tenant; record only); queue calls export `execute_on_answer_cuc_agent` through `cc_export_vars` so `agent_recording.lua` runs on the agent's leg at answer, skips a caller already recorded, and asks telephony-config `/fs/recording/:tenantId/agent-answer` (decides with `agentId`, registers to the agent's extension); agent contact routed through OpenSIPs. Console rule editor offers "Queue agent". Tests: `recording-service/test/policy.domain.test.ts`, `internal.routes.test.ts`, telephony-config `recording.fs.test.ts`, `recording.client.test.ts`, `recording.context.test.ts`, console `recordings_test.dart`; live test `tests/sip/test/recording_agent.test.ts` passes: a queue call reaches agent 301 and is recorded once (the test still adds the agent's tier by hand, G-47) |
| S5-15 | Done | Record, stop, pause and resume buttons for live calls (G-111 (3), G-120), with the feature codes' rules: call-control `POST /v1/tenants/:t/calls/:uuid/recording` (`recording.control`, new, private: tenant admin and supervisor) and `/v1/tenants/:t/me/live-calls/:uuid/recording` (`self.recording`, new: own extension only), routed by api-gateway (`call`); call-control reads the owner channel's `cuc_rec_owner`/`cuc_tenant_id`/`cuc_rec_ctx`/`cuc_recording_id` with `uuid_getvar`, asks recording-service `/internal/v1/recordings/control` with an explicit `action` and the person as actor (decides and audits first), then runs `uuid_record` and `uuid_setvar`, and for a pause fires `CUSTOM cuc::recording` with `sendevent` (`recording_control.lua` fires the same after `*2`), which becomes `call.channel.recording_paused`/`_resumed`; telephony-config exports `cuc_rec_controls=on_demand\|pause`, carried to the hub's `LiveCall.controls`; the hub shows `recording: paused` and adds `tenant:{t}:user:{u}:calls` (a person's own legs, by the extension call-control vouches for, and the legs bridged to them; `self.history`, audited). Console: Actions column on Live calls (pending until the feed shows the change, snackbar on refusal), a live-call card with the same buttons on every My phone screen, demo support. Tests: call-control `recording-control.route.test.ts` (exact ESL commands, fake recording-service, permissions, ownership, refusals, H1), `normalize.test.ts`, `channel-handler.test.ts`, `esl-client.test.ts`, `redis-registry.test.ts`; recording-service `control.routes.test.ts`; telephony-config `recording.fs.test.ts`, `recording.context.test.ts`; api-gateway `realtime.test.ts`, `realtime-model.test.ts`, `route-table.test.ts`; authz; console `monitoring_test.dart`, `self_service_test.dart`. Live test `tests/sip/test/recording_live_controls.test.ts` passes (Start/Stop, Pause/Resume of a rule recording with the hub showing `paused` then `on`, refusals); the `*2` path's event is seen live too |
| S5-16 | Done | Voicemail audio reaches storage: `voicemail.lua` records to `vm-<id>.wav` in the spool and leaves it; the node uploader (`recording-service/src/uploader`, `createVoicemailApi`) delivers it to voicemail-service's `routes/upload.routes.ts` (`upload-url`, `complete` verifying size and MD5 against storage, `fail`); pending sweep (`pending-sweep.ts`). Tests: `voicemail-service/test/upload.routes.test.ts` (real MinIO), uploader voicemail cases. The live check in `tests/sip/test/voicemail.test.ts` (audio playable, spool emptied) passes on a fresh stack (G-2) |
