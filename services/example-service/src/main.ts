...
import { createWidgetRepo } from './repo/widget.repo.js';
...
const db = createDatabase<ExampleServiceDb>({
  host: config.DB_HOST,
  port: config.DB_PORT,
  user: config.DB_USER,
...
const widgetRepo = createWidgetRepo(db);
registerWidgetRoutes(app, widgetRepo);
